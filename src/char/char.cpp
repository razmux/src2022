...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if (
		...
		(p->spl != cp->spl) || (p->con != cp->con) || (p->crt != cp->crt) || (p->playtime != cp->playtime)
	)
	{	//Save status
		if( SQL_ERROR == Sql_Query(sql_handle, "UPDATE `%s` SET `base_level`='%d', `job_level`='%d',"
			...
			"`pow`='%d',`sta`='%d',`wis`='%d',`spl`='%d',`con`='%d',`crt`='%d',`playtime`='%u'"
			...
			p->pow, p->sta, p->wis, p->spl, p->con, p->crt,p->playtime,
...
int char_mmo_char_fromsql(uint32 char_id, struct mmo_charstatus* p, bool load_everything) {
	...
	||	SQL_ERROR == SqlStmt_BindColumn(stmt, 73, SQLDT_UINT,   &p->playtime, 0, NULL, NULL)
	)
	{
		SqlStmt_ShowDebug(stmt);
		SqlStmt_Free(stmt);
...
void char_read_fame_list(void)
{
	...
	{
		// char_id
		Sql_GetData(sql_handle, 0, &data, NULL); woe_fame_list[i].id = atoi(data);
		// fame
		Sql_GetData(sql_handle, 1, &data, &len); woe_fame_list[i].fame = atoi(data);
		// name
		Sql_GetData(sql_handle, 2, &data, &len); memcpy(woe_fame_list[i].name, data, min(len, NAME_LENGTH));
	}
	Sql_FreeResult(sql_handle);
}
...
{
	unsigned char buf[4];
	char* data, name[NAME_LENGTH];
	size_t len;
	uint32 char_id = 0;
	struct item it;

	ShowInfo("Ranking Reset type: (%d) Request from Map Server...\n", type);
	switch( type )
	{
	...
	case RANK_BG:
		...
		if (SQL_ERROR == Sql_Query(sql_handle, "TRUNCATE TABLE `bg_skill_count`"))
			Sql_ShowDebug(sql_handle); // Kill log cleanup
		break;
